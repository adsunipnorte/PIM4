...
namespace PIM3.Desktop
{
    public partial class FrmTpCliente : Form
    {
        ...
        private void FrmTpCliente_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            if (rdbsituacao.Checked == true) // Verifica se radiobutton descrição está marcado, sendo verdadeiro coloca o foco no textbox pesquisar
            {
        ...
        {
            if (rdbdescricao.Checked == true) // Verifica se radiobutton ID está marcado, caso esteja coloca o foco no maskedtextbox pesquisar
            {
        ...
        private void btnpesquisar_Click(object sender, EventArgs e)
        {
            ...
            else if (rdbdescricao.Checked == true) // Caso radiobutton ID esteja marcado, faz select pesquisando por número contido no maskedtextbox
            {
                ...
                string Query = "select tb_tipocliente.id, tb_tipocliente.descricao as DESCRICAO, tb_situacao.descricao as SITUACAO from tb_tipocliente inner join tb_situacao on tb_situacao.id=tb_tipocliente.situacao where upper(tb_tipocliente.descricao)  like '%" + txtpesquisar.TextName + "%'";
            ...
            else if (rdbsituacao.Checked == true) // Caso radiobutton esteja marcado, faz pesquisa pela descrição contida no textbox
            {
                ...
                string Query = "select tb_tipocliente.id, tb_tipocliente.descricao as DESCRICAO, tb_situacao.descricao as SITUACAO from tb_tipocliente inner join tb_situacao on tb_situacao.id=tb_tipocliente.situacao where upper(tb_situacao.descricao) like '%" + txtpesquisar.TextName + "%'";
