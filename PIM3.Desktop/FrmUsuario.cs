...
namespace PIM3.Desktop
{
    public partial class FrmUsuario : Form
    {
        ...
        {
            msktxtpesquisa.Location = new Point(488, 70); // Coloca maskedtextbox na mesma posição do textbox descrição
        }
        ...
        private void btnpesquisar_Click(object sender, EventArgs e)
        {
            ...
            else if (rdbid.Checked == true) // Caso radiobutton ID esteja marcado, faz select pesquisando por número contido no maskedtextbox
            {
                ...
                string Query = "select tb_usuarios.id, tb_usuarios.login, tb_situacao.descricao, tb_usuarios.email from tb_usuarios left join tb_situacao on tb_situacao.id=tb_usuarios.situacao where tb_usuarios.id =" + Convert.ToString(msktxtpesquisa.Text);
            ...
            else if (rdbdescricao.Checked == true) // Caso radiobutton esteja marcado, faz pesquisa pela descrição contida no textbox
            {
                ...
                string Query = "select tb_usuarios.id, tb_usuarios.login, tb_situacao.descricao, tb_usuarios.email from tb_usuarios left join tb_situacao on tb_situacao.id=tb_usuarios.situacao where upper(tb_usuarios.login) like '%" + txtpesquisar.TextName + "%'";
